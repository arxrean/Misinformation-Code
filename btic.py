...
class MySet(Dataset):
	...
	def __getitem__(self, idx: int):
		...
		img_feat = self.transform(Image.open(row['imgpath']).convert('RGB')).reshape(3, 8*8, 80, 80).transpose(0, 1)
...
class MyModel(nn.Module):
	def __init__(self, opt):
		...
		self.transformer = nn.Transformer(32, batch_first=True)

		self.multi_atten = nn.MultiheadAttention(opt.hid_dim, opt.heads, batch_first=True)
		...
		self.image_fc1 = nn.Linear(2048,  self.hidden_size)
	...
	def forward(self, text, mask, image):
		### IMAGE #####
		image = self.resnet50(image.reshape(-1, *image.shape[2:]))
		image = image.reshape(-1, 64, *image.shape[1:])
		image = F.leaky_relu(self.image_fc1(image.mean(-1).mean(-1)))
		image = self.transformer(image, image)
		
		##########CNN##################
		text = self.bert(text, attention_mask=mask).last_hidden_state
		text = self.bert_fc(text)

		text_image = torch.cat((text, image), 1)
		text_image = self.multi_atten(text_image, text_image, text_image)[0]

		### Fake or real
		class_output = self.class_classifier(torch.cat((torch.mean(text_image, 1), torch.max(text_image, 1)[0]), 1))
		## Domain (which Event )
...
# python btic_baseline.py --epoches 20 --num_workers 4 --batch_size 128  --lr 1e-4 --data_path ./repo/recovery.csv
if __name__ == '__main__':
	...
	torch.save(model.state_dict(), './save/btic.pth.tar')